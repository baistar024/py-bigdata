...
class Frame1(wx.Frame):
    def __init__(self, parent):
        ...
        self.grid1.Bind(wx.grid.EVT_GRID_CELL_CHANGED,
    ...
    def OnGrid1GridCellChange(self, event):
        ...
        print
        'Changed cell: (%u, %u)' % (Row, Col)
        print
        'value: %s' % self.grid1.GetCellValue(Row, Col)

        # Row 0 means a GridCellChoiceEditor, so we should have associated
        # an index and client data.
        if Row == 0:
            print
            'index: %u' % self.grid1.index
            print
            'data: %s' % self.grid1.data

        print
        ''  # blank line to make it pretty.
        event.Skip()
    ...
    def OnGrid1ComboBox(self, event):
        ...
        print
        'ComboBoxChanged: %s' % self.comboBox.GetValue()
        print
        'ComboBox index: %u' % self.grid1.index
        print
        'ComboBox data: %u\n' % self.grid1.data
        event.Skip()
    ...
    def OnGrid1ComboBoxText(self, event):
        ...
        print
        'ComboBoxText: %s' % self.comboBox.GetValue()
        print
        'ComboBox index: %u\n' % self.grid1.index
        event.Skip()
